...
import Door.Access.Door8800.Command.Door.LockDoor;
 ...
 */
@Component
public class LockDoorService extends ControlService {

    private static final Logger log = LoggerFactory.getLogger(LockDoorService.class);

    public LockDoorService() {
        super(DoorOperation.LOCK_DOOR);
    }
     ...
     * @param deviceList
     */
    public void lockDoor(List<Device> deviceList) {
        deviceList.stream().forEach(device -> {
            ...
            if (isControl(device)) {
                ...
                Face.Door.LockDoor cmd = new Face.Door.LockDoor(par);
                //添加命令到队列
                _Allocator.AddCommand(cmd);
            }
        });
