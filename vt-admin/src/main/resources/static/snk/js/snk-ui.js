...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			imageView: function (value, height, width, target) {
				...
				if ($.common.isNotEmpty(value)) {
					return $.common.sprintf("<img class='img-circle img-xs' data-height='%s' data-width='%s' data-target='%s' src='%s'/>", height, width, _target, ctx + value);
				} else {
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, callback, noBtn) {
            	//如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    width = 'auto';
            	    height = 'auto';
            ...
            // 弹出层全屏
            openFull: function (title, url, width, height, noBtn, shadeClose) {
            	//如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    width = 'auto';
            	    height = 'auto';
        ...
        operate: {
            ...
            // 添加信息
            add: function(id, noBtn) {
            	table.set();
            	$.modal.open("添加" + table.options.modalName, $.operate.addUrl(id), null, null, null, noBtn);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
            	table.set();
            ...
            edit: function(id, noBtn) {
            	...
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("修改" + table.options.modalName, url,  null, null, null, noBtn);
            	} else {
            	    $.modal.open("修改" + table.options.modalName, $.operate.editUrl(id), null, null, null, noBtn);
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.snk_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
